d3.sankey = function() {
  ...
  function computeNodeDepths(iterations) {
    ...
    function resolveCollisions() {
      nodesByBreadth.forEach(function(nodes) {
        ...
        // nodes.sort(ascendingDepth); //禁止排序的关键代码
        for (i = 0; i < n; ++i) {
          node = nodes[i];
          dy = y0 - node.y;
          if (dy > 0) node.y += dy;
  ...
  function computeLinkDepths() {
    nodes.forEach(function(node) {
      node.sourceLinks.sort(ascendingSourceDepth); // ascendingTargetDepth//typeSort
      node.targetLinks.sort(ascendingTargetDepth); //was ascendingTargetDepth//typeSort
    });
    nodes.forEach(function(node) {
      var sy = 0,
        ty = 0;
